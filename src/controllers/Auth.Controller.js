...
const verifyOtpController = async (req, res) => {
  try {
    ...
    if (new Date( Date.now() ) > storedOtp.expiresAt) {
   ...
   const token =  jwt.sign({_id:storedOtp._id},process.env.JWT_SECRET_KEY,{expiresIn:'7d'})

      storedOtp.status="active"
      storedOtp.save()

   res.status(201).json({status:"success",message:"Registration successful",token:token})
